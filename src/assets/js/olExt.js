 ...
 * @description ol3,openlayers,Kopup,menu,symbol,renderer,filter,canvas,interaction,split,statistic,charts,pie,LayerSwitcher,toolbar,animation
 * @version v3.1.2
...
ol.control.LayerSwitcher.prototype.switchLayerVisibility = function(l, layers) {
  ...
  if (!l.get('baseLayer')) {
	...
	if (l.getVisible() == true) {
	    $('#leg_end').append(l.H.legend);

	}
	
	else if ((l.getVisible() == false)){
		
		$('#leg_'+ l.H.id).remove();
		// popup.hide();
		
		//ShowLabel(l.H.uniName, false)
...
ol.control.LayerSwitcher.prototype.drawList = function(ul, collection) {
  ...
  for (var i=layers.length-1; i>=0; i--) {
	...
	if (this.hasOpt && layer instanceof ol.layer.Tile && layer.get('baseLayer') != true) {
		
    ol.ext.element.create('LABEL', {
      html: "<i class='fa fa-ellipsis-v' aria-hidden='true' style='padding-top: 8px;' onClick = ShowLabel('"+layer.get("uniName")+"',"+layer.get("visible")+")></i>",
 ...
 if (this.hasOpt && layer instanceof ol.layer.Tile) {
 ...
 * @param {Object=} options Control options.
 */
ol.control.LayerKopup = function(options) {
  options = options || {};
	options.switcherClass="ol-layerswitcher-Kopup";
	if (options.mouseover!==false) options.mouseover=true;
	ol.control.LayerSwitcher.call(this, options);
};
ol.ext.inherits(ol.control.LayerKopup, ol.control.LayerSwitcher);
/** Disable overflow
*/
ol.control.LayerKopup.prototype.overflow = function(){};
/** Render a list of layer
 * @param {elt} element to render
 * @layers {Array{ol.layer}} list of layer to show
 * @api stable
 */
ol.control.LayerKopup.prototype.drawList = function(ul, layers) {	
  var self=this;
	var setVisibility = function(e) {
    e.preventDefault(); 
...
{	var options = opt_options || {};
	...
	this.Kopup_ = document.createElement("div");
	this.Kopup_.classList.add("ol-profilKopup");
	this.cursor_.appendChild(this.Kopup_);
	// Track information
...
/** Show Kopup info
* @param {string} info to display as a Kopup
* @api stable
*/
ol.control.Profil.prototype.Kopup = function(info)
{	this.Kopup_.innerHTML = info;
}
...
{	if (!this.tab_.length) return;
	...
	{	this.bar_.style.left = dx+"px";
		...
		if (dx>this.canvas_.width/ratio/2) this.Kopup_.classList.add('ol-left');
		else this.Kopup_.classList.remove('ol-left');
...
*/
/**
 * Select features by property using a Kopup 
 *
 * @constructor
...
/** Set the Kopup values
 * @param {Object} options
 *  @param {Object} options.values a key/value list with key = property value, value = title shown in the Kopup, default search values in the sources
 ...
 *  @param {function|undefined} options.onchoice function triggered when an option is clicked, default doSelect
 */
ol.control.SelectKopup = function(options) {
  if (!options) options = {};
  // Container
  var div = options.content = ol.ext.element.create('DIV');
  if (options.label) {
  ...
  options.className = options.className || 'ol-select-Kopup';
  ol.control.SelectBase.call(this, options);
  this.set('property', options.property || 'name');
  this.set('max', options.max || 10000);
  this.set('defaultLabel', options.defaultLabel);
...
ol.ext.inherits(ol.control.SelectKopup, ol.control.SelectBase);
/**
* Set the map instance the control associated with.
* @param {o.Map} map The map instance.
*/
ol.control.SelectKopup.prototype.setMap = function(map) {
  ol.control.SelectBase.prototype.setMap.call(this, map);
  this.setValues();
};
/** Select features by attributes
 */
ol.control.SelectKopup.prototype.doSelect = function(options) {
  options = options || {};
...
/** Set the Kopup values
 * @param {Object} values a key/value list with key = property value, value = title shown in the Kopup, default search values in the sources
 */
ol.control.SelectKopup.prototype.setValues = function(options) {
  options = options || {};
  var values, vals;
  if (options.values) {
    if (options.values instanceof Array) {
...
/*	Copyright (c) 2016 Jean-Marc VIGLINO, 
	...
	(http://www.cecill.info/licences/Licence_CeCILL-B_V1-en.txt).
*/
/** Modify interaction with a Kopup to delet a point on touch device
 * @constructor
 * @fires showKopup
 * @fires hideKopup
 ...
 *  @param {Boolean|undefined} options.useKopup use a Kopup, default true
 */
ol.interaction.ModifyTouch = function(options) {
  var self = this;
  if (!options) options = {};
  this._Kopup = new ol.Overlay.Kopup ({
    KopupClass: options.calssName || 'modifytouch',
  ...
  options.condition = function(e) {
    ...
    // Prevent click on the Kopup
    e.preventDefault();
    e.stopPropagation();
		return true;
  };
  // Hide Kopup on insert
  ...
  // Use a Kopup ?
  this.set('useKopup', options.useKopup !== false);
};
...
ol.interaction.ModifyTouch.prototype.setMap = function(map) {	
  if (this.getMap()) {
    this.getMap().removeOverlay(this._Kopup);
  }
	ol.interaction.Modify.prototype.setMap.call (this, map);
  if (this.getMap()) {
    this.getMap().addOverlay(this._Kopup);
  }
};
/** Activate the interaction and remove Kopup
 * @param {Boolean} b
 */
...
ol.interaction.ModifyTouch.prototype.showDeleteBt = function(e) {
  if (this.get('useKopup') && e.type==='show') {
    this._Kopup.show(e.coordinate, this._menu);
  } else {
    this._Kopup.hide();
  }
  e.type += 'Kopup';
  this.dispatchEvent(e);
  // Date if Kopup start a timeout to prevent touch + click on the Kopup
  this._timeout = new Date();
};
/**
 * Change the Kopup content
 * @param {DOMElement} html 
 */
ol.interaction.ModifyTouch.prototype.setKopupContent = function(html) {
  this._menu = html;
}
/**
 * Get the Kopup content
 * @return {DOMElement}
 */
ol.interaction.ModifyTouch.prototype.getKopupContent = function() {
  return this._menu;
}
...
/**
 * @classdesc
 * A Kopup element to be displayed over the map and attached to a single map
 * location. The Kopup are customized using CSS.
 *
 * @example
var Kopup = new ol.Overlay.Kopup();
map.addOverlay(Kopup);
Kopup.show(coordinate, "Hello!");
Kopup.hide();
*
* @constructor
* @extends {ol.Overlay}
* @param {} options Extend Overlay options 
*	@param {String} options.KopupClass the a class of the overlay to style the Kopup.
*	@param {bool} options.closeBox Kopup has a close box, default false.
*	@param {function|undefined} options.onclose: callback function when Kopup is closed
*	@param {function|undefined} options.onshow callback function when Kopup is shown
*	@param {Number|Array<number>} options.offsetBox an offset box
*	@param {ol.OverlayPositioning | string | undefined} options.positionning 
*		the 'auto' positioning var the Kopup choose its positioning to stay on the map.
* @api stable
*/
ol.Overlay.Kopup = function (options) {
  var self = this;
  options = options || {};
  ...
  this.setKopupClass(options.KopupClass || options.className || 'default');
  // Show Kopup on timeout (for animation purposes)
  if (options.position) {
    setTimeout(function(){ this.show(options.position); }.bind(this));
  }
};
ol.ext.inherits(ol.Overlay.Kopup, ol.Overlay);
/**
 * Get CSS class of the Kopup according to its positioning.
 * @private
 */
ol.Overlay.Kopup.prototype.getClassPositioning = function () {
  var c = "";
  var pos = this.getPositioning();
  if (/bottom/.test(pos)) c += "ol-Kopup-bottom ";
  if (/top/.test(pos)) c += "ol-Kopup-top ";
  if (/left/.test(pos)) c += "ol-Kopup-left ";
  if (/right/.test(pos)) c += "ol-Kopup-right ";
  if (/^center/.test(pos)) c += "ol-Kopup-middle ";
  if (/center$/.test(pos)) c += "ol-Kopup-center ";
  return c;
};
/**
 * Set a close box to the Kopup.
 * @param {bool} b
 * @api stable
 */
ol.Overlay.Kopup.prototype.setClosebox = function (b) {
  this.closeBox = b;
  if (b) this._elt.classList.add("hasclosebox");
  else this._elt.classList.remove("hasclosebox");
};
/**
 * Set the CSS class of the Kopup.
 * @param {string} c class name.
 * @api stable
 */
ol.Overlay.Kopup.prototype.setKopupClass = function (c) {
  this._elt.className = "";
    var classesPositioning = this.getClassPositioning().split(' ')
      .filter(function(className) {
        return className.length > 0;
      });
    var classes = ["ol-Kopup"];
    if (c) {
...
/**
 * Add a CSS class to the Kopup.
 * @param {string} c class name.
 * @api stable
 */
ol.Overlay.Kopup.prototype.addKopupClass = function (c) {
  this._elt.classList.add(c);
};
/**
 * Remove a CSS class to the Kopup.
 * @param {string} c class name.
 * @api stable
 */
ol.Overlay.Kopup.prototype.removeKopupClass = function (c) {
  this._elt.classList.remove(c);
};
/**
 * Set positionning of the Kopup
 * @param {ol.OverlayPositioning | string | undefined} pos an ol.OverlayPositioning 
 * 		or 'auto' to var the Kopup choose the best position
 * @api stable
 */
ol.Overlay.Kopup.prototype.setPositioning = function (pos) {
  if (pos === undefined)
    return;
  if (/auto/.test(pos)) {
 ...
 */
ol.Overlay.Kopup.prototype.setPositioning_ = function (pos) {
  if (this._elt) {
    ol.Overlay.prototype.setPositioning.call(this, pos);
    this._elt.classList.remove("ol-Kopup-top", "ol-Kopup-bottom", "ol-Kopup-left", "ol-Kopup-right", "ol-Kopup-center", "ol-Kopup-middle");
...
/** Check if Kopup is visible
* @return {boolean}
*/
ol.Overlay.Kopup.prototype.getVisible = function () {
  return this._elt.classList.contains("visible");
};
/**
 * Set the position and the content of the Kopup.
 * @param {ol.Coordinate|string} coordinate the coordinate of the Kopup or the HTML content.
 * @param {string|undefined} html the HTML content (undefined = previous content).
 * @example
var Kopup = new ol.Overlay.Kopup();
// Show Kopup
Kopup.show([166000, 5992000], "Hello world!");
// Move Kopup at coord with the same info
Kopup.show([167000, 5990000]);
// set new info
Kopup.show("New informations");
* @api stable
*/
ol.Overlay.Kopup.prototype.show = function (coordinate, html) {
 ...
 * @api stable
 */
ol.Overlay.Kopup.prototype.hide = function () {
 ...
 *
 * @example
var Kopup = new ol.Overlay.Placemark();
map.addOverlay(Kopup);
Kopup.show(coordinate);
Kopup.hide();
*
* @constructor
* @extends {ol.Overlay}
* @param {} options Extend ol/Overlay/Kopup options 
...
ol.Overlay.Placemark = function (options) {
  options = options || {};
  options.KopupClass = (options.KopupClass || '') + ' placemark anim'
  options.positioning = 'bottom-center',
  ol.Overlay.Kopup.call(this, options);
...
ol.ext.inherits(ol.Overlay.Placemark, ol.Overlay.Kopup);
/**
...
ol.Overlay.Placemark.prototype.show = function(coordinate, html) {
  ...
  this.hide();
  ol.Overlay.Kopup.prototype.show.apply(this, [coordinate, html]);
};
/**
...
ol.Overlay.Placemark.prototype.setClassName = function(name) {
  ...
  this.element.className = 'ol-Kopup placemark ol-Kopup-bottom ol-Kopup-center ' 
...
*/
/**
 * A Kopup element to be displayed on a feature.
 *
 * @constructor
 * @extends {ol.Overlay.Kopup}
 * @param {} options Extend Kopup options 
 *  @param {String} options.KopupClass the a class of the overlay to style the Kopup.
 *  @param {bool} options.closeBox Kopup has a close box, default false.
 *  @param {function|undefined} options.onclose: callback function when Kopup is closed
 *  @param {function|undefined} options.onshow callback function when Kopup is shown
 ...
 *  @param {ol.OverlayPositioning | string | undefined} options.positionning 
 *    the 'auto' positioning var the Kopup choose its positioning to stay on the map.
 *  @param {*} options.template A template with a list of properties to use in the Kopup
 *  @param {boolean} options.canFix Enable Kopup to be fixed, default false
 ...
 *  @api stable
 */
ol.Overlay.KopupFeature = function (options) {
  options = options || {};
  ol.Overlay.kopup.call(this, options);
...
ol.ext.inherits(ol.Overlay.KopupFeature, ol.Overlay.Kopup);
/** Set the template
 * @param {*} template A template with a list of properties to use in the kopup
 */
ol.Overlay.KopupFeature.prototype.setTemplate = function(template) {
...
/** Show the kopup on the map
 * @param {ol.coordinate|undefined} coordinate Position of the kopup
 * @param {ol.Feature|Array<ol.Feature>} features The features on the kopup
 */
ol.Overlay.KopupFeature.prototype.show = function(coordinate, features) {
  ...
  if (html) {
    ...
    ol.Overlay.Kopup.prototype.show.call(this, coordinate, html);
  }
};
/**
 * @private
 */
ol.Overlay.KopupFeature.prototype._getHtml = function(feature) {
  if (!feature) return '';
  var html = ol.ext.element.create('DIV', { className: 'ol-Kopupfeature' });
  if (this.get('canFix')) {
  ...
  if (this._features.length > 1) {
    ...
    ol.ext.element.create('DIV', { className: 'ol-prev', parent: div })
      .addEventListener('click', function() {
        ...
        ol.Overlay.Kopup.prototype.show.call(this, this.getPosition(), html);
      }.bind(this));
 ...
 */
ol.Overlay.KopupFeature.localString = function (locales , options) {
  return function (a) {
...
ol.Overlay.Tooltip = function (options) {
  options = options || {};
  options.KopupClass = options.KopupClass || options.className || 'tooltips black';
...
ol.ext.inherits(ol.Overlay.Tooltip, ol.Overlay.Kopup);
/**
 * Set the map instance the control is associated with
 * and add its controls associated to this map.
 * @param {_ol_Map_} map The map instance.
...
ol.Overlay.Tooltip.prototype.setMap = function (map) {
  ...
  ol.Overlay.Kopup.prototype.setMap.call(this, map);
  if (this.getMap()) this.getMap().addInteraction(this._interaction);
};
/** Show the kopup. If a feature has been passed to the 
...
ol.Overlay.Tooltip.prototype.show = function(coord, html) {
  ...
  // Show kopup
  ol.Overlay.Kopup.prototype.show.call(this, coord, html);
};
